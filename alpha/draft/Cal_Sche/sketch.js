...
function setup() {
  ...
  titleTx.setAttribute('placeHolder', 'ここに予定を入力');
  ...
  var getDisplayString = function() {
    return date.year + '/' + date.md.substring(0, 2) + '/' + date.md.substring(2, 4) + ' ' +
  ...
  document.addEventListener('keydown', function(event) {
    ...
    if (48 <= event.keyCode && event.keyCode <= 57) {
      ...
      if (selectedIndex != -1) {
        if (event.shiftKey) {
          selectedIndex = (selectedIndex + focusable.length - 1) % focusable.length;
        } else {
  ...
  var doLayout = function(width, height) {
    ...
    !function() {
      ...
      var layoutSelection = function(sel, x, y, width, height) {
        ...
        elm.style.backgroundColor = error? 'rgba(255,0,0,0.2)' : 'rgba(0,0,0,0.1)';
        elm.style.border = lbdr + 'px solid ' + (selected? 'rgba(255,0,127,0.8)':  'rgba(0,0,0,0)');
    ...
    !function() {
      ...
      appendVData('DTSTAMP:' + lzpad(now.getFullYear(), 4) +
                  lzpad(now.getMonth() + 1, 2) +
                  lzpad(now.getDate(), 2) + 'T000000');
      appendVData('DTSTART:' + date.year + date.md + 'T' + date.sTime + '00');
      appendVData('DTEND:' + date.year + date.md + 'T' + date.eTime + '00');
      appendVData('SUMMARY:' + (titleTx.value || '無題の予定') );
      ...
      tran += 'scale(' + scale + ')';
      tran += translate(qsize / 2, qsize / 2);
      tran += translate(left / scale - qsize / 2, (ttop - lbdr - lgap + top) / 2 / scale - qsize / 2);
